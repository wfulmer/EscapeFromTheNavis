...
draw_text(camera_get_view_x(view_camera[0])+10,camera_get_view_y(view_camera[0])+60,"Score: "+string(curr_score))
}

if(game_over == true){
	draw_text_color(camera_get_view_x(view_camera[0])+416,camera_get_view_y(view_camera[0])+288,"GAME OVER\nPress Alt + R to return to menu",c_white,c_white,c_white,c_white,1);
}